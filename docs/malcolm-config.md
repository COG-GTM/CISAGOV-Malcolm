...
* **`dashboards-secret.env`** - secret values used by the containers that configure and maintain OpenSearch and OpenSearch Dashboards
    - `MALCOLM_API_LOOPBACK_TOKEN` - a shared secret used to authenticate "internal" calls to the Malcolm API [event logging](api-event-logging.md) webhook (e.g., the OpenSearch Alerting loopback webhook); if left blank (the default) no internal bypass is allowed and the webhook is subject to the same authentication/authorization as other API requests
